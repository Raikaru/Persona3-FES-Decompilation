...
class BoundaryReconciliationTests(unittest.TestCase):
    ...
    def test_original_terminal_window_is_not_extended_across_gap(self):
        # The original window ends at 0x1020. verify.py derives an upper bound
        # from `entry + window`, so the marker at 0x1010 is sized against that
        # end, not against the next entry across the unmapped gap.
        ...
        self.assertEqual(windows["00001010"], 0x10)
