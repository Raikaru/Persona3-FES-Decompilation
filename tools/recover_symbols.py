...
def non_linkable_symbols(symbols):
    """Return compiler intrinsics and null targets that cannot be linker symbols."""
    return {
        name: address
        for name, address in symbols.items()
        if address == 0 or name.startswith("__builtin_")
    }
...
def main():
    ...
    non_linkable = non_linkable_symbols(consistent)
    for name in name_mismatches.keys() | non_linkable.keys():
        del consistent[name]
    lines = [
    ...
    sys.stderr.write(
        ...
        f"conflicts={len(conflicts)} name_mismatches={len(name_mismatches)} "
        f"non_linkable={len(non_linkable)} -> {OUT.name}\n"
    )
    if conflicts:
    ...
    if non_linkable:
        sys.stderr.write(
            "  dropped (non-linkable): "
            + ", ".join(
                f"{name}={address:#x}" for name, address in sorted(non_linkable.items())
            )
            + "\n"
        )
