...
def _wslpath(path: Path | str) -> str:
    p = subprocess.run(
        ["wsl", "-d", os.environ.get("P3_WSL_DISTRO", "Debian"), "--", "wslpath", "-a", str(path).replace("\\", "/")],
...
def _wsl_has(cmd: str) -> bool:
    return subprocess.run(
        ["wsl", "-d", os.environ.get("P3_WSL_DISTRO", "Debian"), "--", "sh", "-lc", f"command -v {shlex.quote(cmd)} >/dev/null"],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ).returncode == 0
...
def find_gnu_tool(name: str, env_name: str) -> Tool:
    ...
    if _which(name):
        return Tool((name,))
    if _wsl_has(name):
        return Tool(("wsl", "-d", os.environ.get("P3_WSL_DISTRO", "Debian"), "--", name), wsl=True)
    sys.exit(
...
if __name__ == "__main__":
    ...
    asm_tool = find_gnu_tool("mipsel-linux-gnu-as", "P3_AS")
    objcopy_tool = find_gnu_tool("mipsel-linux-gnu-objcopy", "P3_OBJCOPY")
