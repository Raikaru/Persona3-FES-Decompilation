...
Two uses:
  ...
  * `--upload` creates a scratch so a stuck function can be shared, and so OTHER
    ...
    P4 is a mixed-toolchain build; this port asks the same question of P3FES.
...
Usage:
  python tools/decompme.py --asm 001b02c0
  python tools/decompme.py --upload 001b02c0 --source my_body.c --context ctx.h
  python tools/decompme.py --compilers ps2
  python tools/decompme.py --try 001b02c0 --source body.c --context ctx.h \
...
def _retail():
    ...
    return verify.RetailElf(cfg["retail_elf"], sizes["sha1"]), sizes
...
def main():
    ...
    asm = retail_asm(addr)
    d = create(args.name or f"P3FES func_{addr}", asm, source, context,
