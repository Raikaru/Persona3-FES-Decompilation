...
typedef struct SiteibiEvent
{
    ...
    u16 flagReqOn1;     // 0x0e. first 'FLG_*' that needs to be true
    u16 flagReqOn2;     // 0x10. second 'FLG_*' that needs to be true
    u16 flagReqOff1;    // 0x12. first 'FLG_*' that needs to be false
    u16 flagReqOff2;    // 0x14. second 'FLG_*' that needs to be false
    u16 flagToActivate; // 0x16. 'FLG_*' that the event needs to activate
} SiteibiEvent;
