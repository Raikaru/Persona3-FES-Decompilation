                    ...
                    const void* triangle, FldFrameRaycast* raycast)
{
    ...
    candidate = (const FldFrameCollisionTriangle*)triangle;
    segmentX = lineCopy.point[0].x - lineCopy.point[1].x;
    segmentY = lineCopy.point[0].y - lineCopy.point[1].y;
    segmentZ = lineCopy.point[0].z - lineCopy.point[1].z;
    normalY = candidate->normal.y;
    normalX = candidate->normal.x;
    normalZ = candidate->normal.z;
    vertexZ = candidate->vertices[0]->z;
    vertexX = candidate->vertices[0]->x;
    vertexY = candidate->vertices[0]->y;
    denominator = normalY * segmentY;
    denominator += normalX * segmentX;
    denominator += normalZ * segmentZ;
    vertexDot = normalZ * vertexZ;
    vertexDot += normalX * vertexX;
    vertexDot += normalY * vertexY;
    ...
    raycast->hitPointDst->x = lineCopy.point[0].x + segmentX * fraction;
    raycast->hitPointDst->y = lineCopy.point[0].y + segmentY * fraction;
    raycast->hitPointDst->z = lineCopy.point[0].z + segmentZ * fraction;
    raycast->didHit = true;
    return NULL;
}
